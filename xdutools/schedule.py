...
URLS = {
    'kb':
    'http://ehall.xidian.edu.cn/jwapp/sys/wdkb/modules/xskcb/xsllsykb.do',
}
APP_NAME = '我的课表'
...
class Lesson:
    ...
    def get_lessons(self, session):
        ...
        return map(self.process_lesson,
    ...
    def process_week(self, week):
        ...
        for w in week.split(','):
            ...
            if m:
                rst.extend(i for i in range(int(m.group(1)),
                                            int(m.group(2)) + 1, 2))
                continue
    ...
    def save_lessons_as_csv(self, lessons):
        with open('./schedule.csv', 'w', encoding='utf-8') as fo:
            ...
            fo.writelines(
                ','.join((i['课程'], i['教师'], self.week[i['周几']], '%d-%d' %
                          (i['开始节数'], i['结束节数']), i['教室'],
                          ' '.join(map(str, i['周数'])))) + '\n'
                for i in lessons)
    ...
    def save_lessons_as_wakeup(self, lessons):
        ...
        with open('./schedule-wakeup.csv', 'w', encoding='utf-8') as fo:
            ...
            fo.writelines(','.join((i['课程'], str(i['周几']), str(i['开始节数']),
                                    str(i['结束节数']), i['教师'], i['教室'],
                                    '、'.join(map(str, i['周数'])))) + '\n'
                          for i in lessons)
